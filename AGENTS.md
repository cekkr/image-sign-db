...
- **`trainImageAdaptive` is the normal path** (`settings.sign.train.adaptive`, default `true`; `--no-adaptive` opts out): it writes in `SIGN_TRAIN_CHECK_EVERY` chunks and stops when confident fresh probes no longer improve accuracy or search effort. With no `--constellations`, one chunk is the starting point and `SIGN_TRAIN_EXTEND_TO` (8192) is the finite safety ceiling; validation chooses the density. A supplied count is nominal unless `--extend-to 0` makes it a hard cap. Invariants it must not lose:
  ...
  - **The stop rule may only fire once `SIGN_TRAIN_STOP_MIN_HIT_RATE` of the probes both find the image and reach the search engine's confidence stop.** Rank-1 only after exhausting the probe is not an optimum. This is the one unrecoverable mistake the loop can make: a flat checkpoint below that bar means "not findable yet", not "trained enough".
...
- **Measured outcome, and it is not the one the feature was built for.** On [`sample_images/`](sample_images) the stop rule almost never fires below 2048, and the curves say that is correct. Read `reason: 'exhausted'` as **"still improving when the safety ceiling ran out"**, not as "trained enough". The shipped 8192 `SIGN_TRAIN_EXTEND_TO` follows that evidence; use `0` only for a controlled hard-ceiling experiment. Do not loosen the success gate to manufacture convergence — that previously cost **rank-1 81.6% → 53.1%**.
