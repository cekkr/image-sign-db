...
- [`signStore.js`](src/lib/cheetah/signStore.js) — storage and retrieval for [`src/lib/sign/`](src/lib/sign): `putImage`, `putSigns`, `commitGraph`, `updateImage`, `markComplete`, `listImages`, `wordDegrees`, `recallImages`, `signsForWord`, `getSign`. Same commit-marker protocol as `CheetahStore` — readers ignore anything without `complete`.
